...
```bash
sh -c "$(curl -sfL https://raw.githubusercontent.com/ablate-ai/aura/main/uninstall.sh)"
```

彻底删除（含 `/etc/aura` 配置）：

```bash
REMOVE_CONFIG=1 sh -c "$(curl -sfL https://raw.githubusercontent.com/ablate-ai/aura/main/uninstall.sh)"
```
